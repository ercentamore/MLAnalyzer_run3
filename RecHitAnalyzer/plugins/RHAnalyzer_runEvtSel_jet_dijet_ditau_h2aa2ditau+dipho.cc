...
void RecHitAnalyzer::branchesEvtSel_jet_dijet_ditau_h2aa2tau2pho ( TTree* tree, edm::Service<TFileService> &fs )
{
...
bool RecHitAnalyzer::runEvtSel_jet_dijet_ditau_h2aa2tau2pho ( const edm::Event& iEvent, const edm::EventSetup& iSetup ) {
  ...
  for (auto iP = photons->begin(); iP != photons->end(); ++iP) {
    if ( iP->pt() > pho_min_pT) && ( reco::deltaR( iP->eta(), iP->phi(), iGen->eta(), iGen->phi() ) < 0.2 ) {
      vPhotons.push_back( iP, std::abs(iP->pt()) )
    }
  };
...
void RecHitAnalyzer::fillEvtSel_jet_dijet_ditau_h2aa2tau2pho ( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  for ( unsigned int iP = 0; iP < vReco_First_Photons_Idxs->size(); iP++ ) {
    ...
    reco::PATObject Photon1( photons, vReco_First_Photons_Idxs[iP] );
    reco::PATObject Photon2( photons, vReco_Second_Photons_Idxs[iP] );
